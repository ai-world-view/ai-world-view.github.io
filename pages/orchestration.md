...
encyclopedic content, one daily *tick* at a time, and publishes its own GitHub
Pages site. **This hub is the brain that drives them all.** It holds every
country's concept, the growth policy, and the agent toolkit; it decides what
grows, how, and when; and it presents the result.

The defining trait of this org: content is written from the model's **own
knowledge** — the AI's own world view. The growth passes do no web research,
fetching, or searching; what is published is what the model itself knows.
...
<svg viewBox="0 0 760 430" role="img" aria-label="The hub holds seeds, policy, framework, and workflows. It dispatches a growth tick to each country repository, which holds only content, a Pages config, thin .claude adapters, and telemetry. Each country repo publishes its own GitHub Pages site, and its seed Evolution Log flows back to the hub." style="max-width:100%;height:auto;font-family:var(--bs-font-sans-serif,system-ui,sans-serif)">
  <defs>
  ...
  <text x="140" y="112" text-anchor="middle" fill="var(--bs-secondary-color,#6c757d)" font-size="11">ai-world-view.github.io</text>
  ...
  <text x="428" y="118" text-anchor="middle" fill="#fff" font-size="14" font-weight="700">COUNTRY REPO &#215;N</text>
  ...
  <text x="668" y="250" text-anchor="middle" fill="var(--bs-secondary-color,#6c757d)" font-size="11">live site / country</text>
...
<figcaption class="figure-caption mt-2">The hub grows each country repo, the repo publishes its own site, and the
record of what grew flows back to the hub.</figcaption>
</figure>

| | The hub (`ai-world-view.github.io`) | Each country repo |
|---|---|---|
...
passes share one working tree, so each builds on the last one's drafts — and
none of them touches the web.

<figure class="my-4 text-center">
<svg viewBox="0 0 820 300" role="img" aria-label="The growth tick pipeline: a daily schedule dispatches a tick; the engine checks out the country repo and stages its seed and framework; a three-tier escalation runs Haiku then Sonnet then Opus; the seed is persisted and hub files stripped; finished content and telemetry are published to the country repo." style="max-width:100%;height:auto;font-family:var(--bs-font-sans-serif,system-ui,sans-serif)">
  <defs>
...
1. **Schedule** — `orchestrate.yml` runs on a daily cron, refreshes the
   ...
   repo (behind a gate that honors the fleet kill-switch).
2. **Prepare** — check out the country repo, then stage its seed
   (`lineage/seeds/<country>.md` → `seed.md`) and the canonical framework
   (`lineage/framework/*` → `.github/`) from the hub.
3. **Generate (Tier 1 · Haiku)** — pick 1–3 topics not yet covered, then write a
   first draft of each from the model's own knowledge — no web research, no
   fetching, no search.
4. **Expand (Tier 2 · Sonnet)** — deepen every draft: detail, specific dates and
   figures the model can recall, and cross-links between topics.
5. **Enhance (Tier 3 · Opus)** — polish prose and check internal consistency,
   rebuild the indices and timeline, regenerate the seed, and append a new
   **Tick N** entry to the seed's Evolution Log.
...
7. **Illustrate** — each new article gets a Claude-authored **SVG preview
   banner** (`scripts/claude_svg_banner.py`; art direction and model from
   `lineage/policy.yml` `preview:`). Without a Claude credential it degrades to
...
<figure class="my-4 text-center">
<svg viewBox="0 0 720 360" role="img" aria-label="A three-step ascending staircase. Tier one is Haiku, which generates drafts by planning, recalling, and writing. Tier two is Sonnet, which expands drafts with detail, dates, and cross-links. Tier three is Opus, which enhances: polishing, building indices, syncing the seed, and finishing ready to publish. Capability and cost increase up the staircase, with an API-key fallback to the side." style="max-width:100%;height:auto;font-family:var(--bs-font-sans-serif,system-ui,sans-serif)">
  <defs>
  ...
  <text x="46" y="304" fill="#000" font-size="11.5">plan · recall · draft new files</text>
  ...
  <text x="261" y="220" fill="#fff" font-size="11.5">detail · dates · cross-links</text>
...
|---|---|---|---|
| 1 · Generate | `claude-haiku-4-5` | Plan the roadmap, write first drafts from its own knowledge | Uncommitted draft files |
| 2 · Expand | `claude-sonnet-4-6` | Deepen each draft — detail, dates, figures, cross-links | Richer drafts |
| 3 · Enhance | `claude-opus-4-8` | Polish, check consistency, rebuild indices, update the seed | Finished content + a new Tick entry |
...
A subtle but load-bearing design choice: the country repos carry only **thin
...
<div class="row g-3 my-3">
  <div class="col-md-4">
    <div class="card h-100"><div class="card-body">
      ...
      <p class="card-text small mb-0">Claude Code finds the skill via the country
...
The effect: the hub is genuinely authoritative for every procedure, the country
repos stay lean (adapters + content only), and a single edit in
`lineage/framework/` reaches every repo on its next tick. This model was proven
in the reference implementation — see
[ADR-0001](https://github.com/year-of-ai/year-of-ai.github.io/blob/main/lineage/decisions/ADR-0001-centralized-growth-orchestration.md)
in the `year-of-ai` hub, from which this org was planted, for the alternatives
weighed and why this one was chosen.
...
- **The seed is the clock.** Each country's seed carries an **Evolution Log**
  (§8). Every published tick appends a `Tick N` entry; that log is how the
  system knows what a repo already covers and how many times it has grown.
...
- **Clean repos by construction.** The strip step guarantees a country repo only
...
- **Never blocked by decoration.** The Illustrate step's SVG banners degrade to
  a deterministic local template without a credential — cover art can fail; a
  publish cannot fail because of it.
...
<div class="row g-3 my-2">
  ...
  <div class="col-md-6">
    <div class="card h-100"><div class="card-body">
      ...
      <p class="card-text small mb-2">Policy, the seeds, and the engine.</p>
      ...
      <a class="btn btn-sm btn-outline-secondary" href="https://github.com/{{ site.repository | join: '' }}/tree/main/lineage/seeds">lineage/seeds/</a>
