...
writes itself also needs to **watch and improve itself**. This page identifies
the routine **agent fleet** that does that: a meta-layer whose job is the health
and continuous improvement of the growth system itself.

The fleet was designed in the reference implementation (the `year-of-ai` hub,
...
|---|---|---|---|
| `telemetry-ledger-collector` | P0 | upgrade | **Built** — appends one record per grow run to the hub evolution ledger (`telemetry/evolution.jsonl`); the P0 keystone every learn/cost/monitor agent depends on |
| `fleet-pause-killswitch` | P0 | net-new | **Built** — `_data/fleet_pause.yml`, the single global halt every dispatching/mutating workflow reads first |
...
| `framework-pr-reviewer` | P0 | upgrade | **Built** — safety for the self-modification path; gates every framework PR |
| `injection-surface-auditor` | P1 | net-new | Audits the workflow-injection surface (untrusted input into prompts; tools-with-write) |
...
| `repo-hygiene-warden` | P1 | net-new | Enforces the invariant that a country repo holds only content + config + .claude + telemetry |
...
| Agent | Pri | Origin | What it does |
|---|---|---|---|
| `docs-warden` | P1 | net-new | **Built** — gates every PR + sweeps `main` so every change is matched by a doc update on the hub's own doc surface |
...
## Defects the design surfaced (fixed here by construction)

The fleet design was run against the reference implementation and surfaced real
defects in the old per-repo model. This hub was **planted after** those findings,
so the fixes arrived with it — each one is worth guarding regardless:

- **No evolution ledger** — per-tick telemetry used to be discarded after 14
  days; here `telemetry-ledger.yml` appends every run to `telemetry/evolution.jsonl`.
- **No supply-chain security** — here `codeql.yml` scans the hub; the fuller
  Dependabot / action-pinning audit remains on the roadmap.
- **A bypassable review** — the old `learn.yml` force-merged its own PR; here
  framework changes are gated by `framework-pr-reviewer.yml`.
- **A publish race** on each member repo's `main` — here every writer joins the
  `repo-write-<repo>` concurrency group.
- **A prompt-injection surface** — here the engine's gate job validates dispatch
  inputs before anything reaches a model prompt.
- **`CLAUDE.md` drift** — member repos can still drift from canon between
  re-syncs; the canon warden that closes this is on the roadmap.

## Rollout order

1. **Phase 0 — safety scaffolding** (before any mutating agent): the kill-switch,
   the write serializer, the gated framework PR review. **Planted built.**
2. **Phase 1 — keystone + gates + canaries:** the telemetry ledger collector,
   then the secret / Pages / health watchers. **Planted built**; the inline
   prepublish gate remains open.
...
4. **Phase 3 — slow structural, economic & meta:** tier-ROI, cost governor,
   content & consistency auditors, recovery rehearsal, and the fleet
   meta-auditor.
